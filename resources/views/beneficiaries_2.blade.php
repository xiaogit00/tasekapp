 ...
 <!-- Beneficiaires Table -->
  ...
  <script>
    ...
    function loadMoreData(page) {
      $.ajax({ // ajax method to perform AJAX request (parameters sent in)
        url: '/' + page, // the URL to send the request to (default is the current page)
        type:'get', // specifies the type of request (get/post)
        beforeSend: function() { // beforeSend method => run before sending the request 
          $(".ajax-load").show(); // show() method shows the hidden element
        }
      })
